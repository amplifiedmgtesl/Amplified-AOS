  ...
  return (
    <div className="grid">
              ...
              {rows.map((row, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td>
                      <select disabled={mode === "none"} value={row.otAfter} onChange={(e) => updateRow(index, { otAfter: e.target.value as TriggerOption })}>
      ...
      <div className="doc-sheet">
        ...
        <div style={{ overflowX: "auto" }}>
          <table>
            <thead><tr><th>Position</th><th>Specialty</th><th>Hr</th><th>Day</th><th>OT</th><th>DT</th><th>OT after</th><th>DT after</th><th>Travel</th></tr></thead>
            <tbody>
              {visibleRows.map((row, idx) => (
                <tr key={idx}>
                  <td>{row.position}</td><td>{row.specialty}</td><td>${row.hourly.toFixed(2)}</td><td>${row.day.toFixed(2)}</td><td>${row.otRate.toFixed(2)}</td><td>${row.dtRate.toFixed(2)}</td><td>{triggerLabel(row.otAfter)}</td><td>{triggerLabel(row.dtAfter)}</td><td>{row.travel ? `$${row.travel.toFixed(2)}` : "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
