...
- **#29 — Proper "ACH Only" payment-terms element on the invoice** (added 2026-07-21, John). Connor currently communicates ACH-only payment by **abusing the `company_settings.phone` field** — prod's `phone` literally holds the string `"ACH ONLY"` (verified 2026-07-21), so it prints in the letterhead contact line (`ACH ONLY · connor@amplifiedesl.com`) on every quote/invoice/pre-invoice report. Fix: add a real, hardcoded/first-class "ACH Only" payment-terms indicator on the **invoice** (Connor wants it) — likely near the remit-to / balance-due block, not the letterhead contact line — so the `phone` field can go back to holding an actual phone number (or be left blank). Decide scope: invoice only vs. also quotes/pre-invoice report; static label vs. a company-settings toggle (`ach_only` boolean) so it's data-driven instead of hardcoded. Touches `invoice-pdf-view.tsx` (and the shared letterhead/remit-to block); `company_settings` already has the bank/remit fields to sit beside. Small, but confirm placement + wording with Connor.
